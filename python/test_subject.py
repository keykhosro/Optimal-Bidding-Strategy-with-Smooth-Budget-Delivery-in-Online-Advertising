# -*- coding: utf-8 -*-
"""
Created on Fri Jun  4 22:46:47 2021

@author: keykhosro
"""
# -*- coding: utf-8 -*-
"""
Created on Tue May 25 02:04:23 2021
...
from rlb_dp_smooth import RLB_DP_SMOOTH
from utility import *
import os
...
c0 = 1 / 8
gamma = 1
#alphas = [0.001,0.0001,0.00001,0.000001,0.0000001,0.00000001,0.000000001]
alphas = [0.1,0.0000001]
src = "ipinyou"
...
for alpha in alphas:
	log_in = open(config.projectPath + "bid-performance/model1_{}_N={}_c0={}_alpha={}_obj={}_clkvp={}.txt".format(src, N, c0, alpha, obj_type, clk_vp), "w")
...
# 		log_in.write(log + "\n")
		...
		# RLB-proj
		#alpha = 0.0000001
		rlb_dp_smooth = RLB_DP_SMOOTH(camp_info, opt_obj, gamma, m_pdf, alpha)
		rlb_dp_smooth.calc_optimal_value_function_with_approximation_i(N, B, max_market_price, m_pdf,
	                                                                   data_path + camp + "/bid-model/smooth_v_nb_N={}_alpha={}.txt".format(
	                                                                       N, alpha))
		auction_in = open(data_path + camp + "/test.theta.txt", "r")
		rlb_dp_smooth = RLB_DP_SMOOTH(camp_info, opt_obj, gamma, m_pdf, alpha)
		setting = "{}, camp={}, algo={}, N={}, c0={}" \
 	        .format(src, camp, "rlb_smooth", N, c0)
		bid_log_path = config.projectPath + "bid-log/{}_{}.txt".format(setting, alpha)
 	    
		model_path = data_path + camp + "/bid-model/smooth_v_nb_N={}_alpha={}.txt".format(N, alpha)
		rlb_dp_smooth.load_value_function(N, B, model_path)
 	    
		(auction, imp, clk, cost) = rlb_dp_smooth.run(auction_in, bid_log_path, N, c0,
 	                                                  max_market_price, delimiter=" ", save_log=True)
		...
		obj = opt_obj.get_obj(imp, clk, cost)
		log = "{:<55}\t {:>10}\t {:>8}\t {:>10}\t {:>8}\t {:>8}\t {:>8.2f}%\t {:>8.2f}\t {:>8.2f}\t {:}" \
 	        .format(setting, obj, auction, imp, clk, cost, win_rate, cpm, ecpc, clk_per_imp)
		print(log)
		log_in.write(log + "\n")
		break
